...
//pake array
cout << "\n Pake Array :";
cout << "\n Data yang anda masukan adalah "; //// hapus baris ini jika tidak ingin menampilkan elemennya lagi
max = min = data[0];
for(i=0; i<a; i++) {
cout << data[i] << ','; // hapus baris ini jika tidak ingin menampilkan elemennya lagi
sum = sum + data[i];
if (max < data[i]) {
max = data[i];
}
